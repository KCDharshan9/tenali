...
5. **Next button + auto-advance.** After an answer is revealed, show a prominent green **Next →** button AND a 5-second countdown ("auto-advance in 5s · or press Enter"). Any keystroke (Enter or Space) or button click advances immediately. Navigation away cancels the timer.
   - **Spaced re-attempts on wrong answers.** When the student answers a question wrongly, re-insert that question into the play sequence a few items later (default: position `qIdx + 4`, clamped to end of list) so they get a second chance to nail it after some distance. The lesson runs against a dynamic `playList` of source-question indices (initialised to `[0..N-1]`); a wrong answer pushes `playList.splice(qIdx + 4, 0, sourceIdx)`, lengthening the list. The slider's max grows to match. A retry that's also wrong gets re-inserted again. The lesson is "complete" only when the playList is exhausted. The student should see a small badge ("retry" or "↻") on questions that came back from the queue, so they know why they're seeing it again.
